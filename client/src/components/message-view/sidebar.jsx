...
function SideBar() {
  const { users, selectedUser, isLoading } = useSelector((state) => state.chat);
  const { user } = useSelector((state) => state.auth);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getUsers(user?._id)).then((data) =>
      console.log(data, "in sidbaaaaar")
    );
  }, [dispatch]);

  console.log(users, "users in sidbar component");
